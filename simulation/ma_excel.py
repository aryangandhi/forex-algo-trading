...
def add_pair_charts(df_ma_res, df_ma_trades, writer):
    ...
    for _, row in df_temp.iterrows():
        dft = df_ma_trades[
            (df_ma_trades.cross == row.cross) & (df_ma_trades.pair == row.pair)
        ]
        dft[cols].to_excel(
...
def create_excel(df_ma_res, df_ma_trades, granularity):
    ...
    filename = f"./simulation/ma_sim_{granularity}.xlsx"
...
def create_ma_res(granularity):
    ...
    create_excel(df_ma_res=df_ma_res, df_ma_trades=df_ma_trades, granularity=granularity)
