...
class OandaApi:
  ...
  def fetch_candles(self, pair_name, count=10, granularity="H1", price="MBA", 
                    date_f=None, date_t=None):
    ...
    ok, response_data = self.make_request(url, verb='get', code=200, params=params, data=None, headers=None)
    data = response_data
  ...
  def create_data_file(self, pair_name, count=10, granularity="H1"):
    ...
    print(f"{pair_name} {granularity} {candles_df.shape[0]} candles")